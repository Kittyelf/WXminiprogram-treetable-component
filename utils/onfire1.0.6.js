...
**   on(event_name, callback)订阅事件   触发多次 
     one(event_name, callback)绑定（订阅）事件 触发一次
     fire(event_name, data) 触发事件
     fireSync(event_name, data)触发同步
     un(event_name/eventObj）取消事件绑定
      clear()清空所有事件
/* jshint expr: true */ 
